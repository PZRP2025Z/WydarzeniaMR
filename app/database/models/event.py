...
Database models for event-related activities.
...
class Event(SQLModel, table=True):
    """
    Database table model representing an event.
    ...
    location: str
    photo: bytes | None = (
        None  # Photo is stored as bytes. We don't have cloud storage, so we save it in DB.
    )
    time: datetime
